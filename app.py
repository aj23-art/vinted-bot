...
def fetch_real_items(per_page=100):
    get_fresh_session()
    if not anon_id:
        return []
    try:
        ...
        for it in items:
            ...
            if price < 5 or price > 80: continue
            ...
            # Lockerere Regeln zum Start - damit du sofort was siehst
            ok = False
...
async function load(){
 ...
 document.getElementById('status').innerText = `Letzter Scan: jetzt | Geprüft: ${data.scanned} Artikel (jetzt 100 pro Scan) | Echte Deals: ${data.count} | Jeder Deal ist live geprüft`;
 list.innerHTML='';
